...
class M_master extends CI_Model{
	...
	function generate_export_move($to,$filename,$sql,$title,$column,$id){
		if ($to == 'csv'){
			$this->load->dbutil(); // call db utility library
			...
			$enclosure = '';//tanda kutip
			//remove firts line (headername)
			$convert = $this->dbutil->csv_from_result_move($sql,$delimiter,$newline,$enclosure,$id);
			force_download($file_name, $convert);
		}else if ($to == 'excel'){
			$this->load->helper('to_excel');
			$file_name = $filename;
			to_excel_move($sql,$file_name,$id);
		}else if ($to == 'pdf'){
			error_reporting(1);
			$parameters = array (
					'paper'=>'A4',
	...
	function generate_export_stock_wh($to,$filename,$sql,$title,$column){
		if ($to == 'csv'){
			$this->load->dbutil(); // call db utility library
			$file_name = $filename.'.csv';
			$delimiter = ";";
			...
			$enclosure = '';//tanda kutip
			//remove firts line (headername)
			$convert = $this->dbutil->csv_from_result_stock_wh($sql,$delimiter,$newline,$enclosure);
			force_download($file_name, $convert);
		}else if ($to == 'excel'){
			$this->load->helper('to_excel');
			$file_name = $filename;
			to_excel_stock_wh($sql,$file_name);
		}else if ($to == 'pdf'){
			error_reporting(1);
			$parameters = array (
					'paper'=>'A4',
			...
			$no = 1;
			foreach ($sql->result_array() as $key=>$value){				
				$siklus = floor($this->m_master->get_total_move(array('product_code'),array('product_code'=>$value['product_code'],'move'=>'Out')));
				$max = $this->m_master->get_stock_max($value['product_code']);		
				$data[$key] = $value;//product code
				$data[$key]['no'] = $no++;
				$data[$key]['in'] = $max['in'];
				$data[$key]['out'] = $max['out'];
				$data[$key]['reject'] = $max['reject'];
				$data[$key]['remove'] = $max['remove'];
				$data[$key]['siklus'] = $siklus;
			}
	...
	function get_total_move($field,$where){
		 ...
		 */
		//tidak menjumlahkan jika ada data out yang double  / discan pada tanggal yang sama walaupun move nya berbeda
		$this->db->group_by('product_code,move,YEAR(date_add),MONTH(date_add),DAY(date_add)');	
		$sql = $this->m_master->get_table_column($field,'tb_stock_take',$where);		
		$total = count($sql);
		$siklus = $total-1;
		/*
		 * jika skilus minus maka akan 0
		 * ditampilkan jika belum melakukan proses scan barang keluar
		 * jika baru 1 kali scan out, maka akan dihitung 0, jika scan > 1 maka akan dihitung siklus 1
		 */
		return ($siklus < 0) ? 0 : $siklus;
	}
	
	function sum_stock($where){		
		$id = (!empty($this->m_master->max_id($where))) ? $this->m_master->max_id($where) : array('');	
		$total=array();	
		foreach ($id as $x){
			$sql = $this->db->select('count(id_stock_take) as total')
						->from('tb_stock_take')
						->where('id_stock_take',$x)
	...
	function get_stock_reject($where='',$column='',$sort='',$length='',$start=''){
		...
		$this->db->group_start();
		foreach ($id as $x){
			$this->db->or_where_in('B.id_stock_take',$x);
		}
		$this->db->group_end();
		$this->db->where('A.deleted',1);				
		$this->db->limit($length,$start);
	...
	function get_stock($where='',$column='',$sort='',$length='',$start=''){			
		...
		$this->db->group_start();
		foreach ($id as $x){
			$this->db->or_where_in('id_stock_take',$x);
		}
		$this->db->group_end();
		$this->db->limit($length,$start);
		$this->db->order_by($column,$sort);			
